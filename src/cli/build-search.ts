...
export async function runBuildSearchCommand(rawArgs: string[]) {
  ...
  if (args.help) {
    console.log(
      'Usage: mdorigin build search --root <content-dir> [--out ./dist/search] [--embedding-backend model2vec|hashing] [--model sentence-transformers/all-MiniLM-L6-v2] [--config <config-file>] [--incremental]',
    );
    return;
  }
  if (!args.root) {
